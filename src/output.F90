...
!     print *, tFR(1,10) 
!#####################################  
      !------------------ Prelim Checking for convergence in local machine if needed -------------------------------------- 
      ref_point = 1
      ref_point2 = 3
      if (n_step==100000) then

      print *, n_step, "reading snapshot and calculating L2 norm of difference in temperature"
      !call read_snapshot2(tFR0,omgFR0,urFR0,upFR0,ref_point2)
      call read_snapshot(dt_new_ref,tot_time_ref,eta_ref,Ra_ref,Pr_ref,Nm_max_ref,Nr_max_ref,rad,tFR_ref, &
                         & omgFR_ref,urFR_ref,upFR_ref,ref_point)  
      print *, "reference time", tot_time_ref
      print *, maxval(aimag(tFR_ref)),  "check" 
      Np_max_ref = 3*Nm_max_ref
      allocate(tFR_comp(Nm_max_ref+1,Nr_max_ref))
      allocate(tFC_comp(Nm_max_ref+1,Nr_max_ref))
      allocate(tt_ref(Np_max_ref,Nr_max_ref))
      allocate(tt_comp(Np_max_ref,Nr_max_ref))
      allocate(temp_diff_phi(Nr_max_ref))
      allocate(ur_diff_phi(Nr_max_ref))
      allocate(ref_diff_phi(Nr_max_ref))

      do i=1,Nm_max+1
         !call chebtransform(Nr_max,tFR0(i,:),ttFC(i,:))
         call chebtransform(Nr_max,tFR(i,:),ttFC(i,:))
      end do
      call destroy_all_fftw_plans()  
      call init_all_fftw_plans(Np_max_ref,Nr_max_ref) 

      if ((Nm_max_ref>Nm_max) .and. (Nr_max_ref>Nr_max)) then
         do i=1,Nm_max+1
            do j=1,Nr_max
               tFC_comp(i,j)=ttFC(i,j)
            end do
         end do
         do i=Nm_max+2,Nm_max_ref
            do j=Nr_max+1,Nr_max_ref
               tFC_comp(i,j)=zero
            end do
         end do
      else if ((Nm_max_ref==Nm_max) .and. (Nr_max_ref>Nr_max)) then
         do i=1,Nm_max+1
            do j=1,Nr_max
               tFC_comp(i,j)=ttFC(i,j)
            end do
...
!----------------------------------------------------------------------------------------------------------------      
      do i=1,Nm_max_ref+1
         call chebinvtran(Nr_max_ref,tFC_comp(i,:),tFR_comp(i,:))
      end do

      do i=1,Nr_max_ref
         tFR_comp(:,i) = tFR_comp(:,i) * sqrt(real(Nr_max_ref-1,kind=dp))/sqrt(real(Nr_max-1,kind=dp))
      end do
      print *, tFR_ref(2,5), tFR_comp(2,4), "check tFR"
      !Inverse fourier to get back real space variables
      do Nr=1,Nr_max_ref  
         call invfft(Nm_max_ref,Np_max_ref,tFR_comp(:,Nr),tt_comp(:,Nr))
      end do
      !Inverse fourier to get back real space variables
      do Nr=1,Nr_max_ref  
         call invfft(Nm_max_ref,Np_max_ref,tFR_ref(:,Nr),tt_ref(:,Nr)) 
      end do
     
      !------ Calculate L2 norm of error for difference in temperature at t point ------------  
      do j=1,Nr_max_ref
          temp_diff_phi(j)=zero
          ur_diff_phi(j)=zero
          ref_diff_phi(j)=zero
          do i=1,Nm_max_ref+1
             !temp_diff_phi(j)=temp_diff_phi(j)+abs(tFR_comp(i,j)-tFR_ref(i,j))*abs(tFR_comp(i,j)-tFR_ref(i,j))
             !ref_diff_phi(j)=ref_diff_phi(j)+abs((tFR_ref(i,j))*(tFR_ref(i,j)))
             temp_diff_phi(j)=temp_diff_phi(j)+abs((tFR(i,j)-tFR_ref(i,j))*(tFR(i,j)-tFR_ref(i,j)))
             ur_diff_phi(j)=ur_diff_phi(j)+abs(urFR(i,j)-urFR_ref(i,j))*abs(urFR(i,j)-urFR_ref(i,j))+&
                                               & abs(upFR(i,j)-upFR_ref(i,j))*abs(upFR(i,j)-upFR_ref(i,j))
          end do
      end do
      
      call radInt(Nr_max_ref,temp_diff_phi,L2error_temp)
      call radInt(Nr_max_ref,ur_diff_phi,L2error_ur)
      L2error_ur = sqrt(L2error_ur)
      L2error_temp = sqrt(2.0_dp*pi*L2error_temp)/pi
      
      print *, n_step, dt_new, tot_time, KE_tot, L2error_temp, L2error_ur, abs(tt_comp(15,15)-tt_ref(15,15))
      ! pad urFR_comp with zeros if you wanna compute Error vs Nm or Nr 
      end if
